(function ($) {
  ...
  //=== CountDownTimer===
	...
	if ($('.coming-soon-countdown').length) {
	  $('.coming-soon-countdown').each(function () {
		var Self = $(this);
		var jalaliDate = Self.data('countdown-time'); // مثال: 1404/07/15
		...
		var miladiDate = new Date(g.gy, g.gm - 1, g.gd, 0, 0, 0); // ساعت 00:00:00 اون روز

		// شمارش معکوس تا تاریخ میلادی
		Self.countdown(miladiDate, function (event) {
		  $(this).html(
			'<li><div class="box"><span class="days">' + event.strftime('%D') + '</span><span class="timeRef">روز</span></div></li>' +
			'<li><div class="box"><span class="hours">' + event.strftime('%H') + '</span><span class="timeRef clr-1">ساعت</span></div></li>' +
			'<li><div class="box"><span class="minutes">' + event.strftime('%M') + '</span><span class="timeRef clr-2">دقیقه</span></div></li>' +
			'<li><div class="box"><span class="seconds">' + event.strftime('%S') + '</span><span class="timeRef clr-3">ثانیه</span></div></li>'
		  );
		});
	  });
	};
  ...
  function priceFilter() {
    if ($(".price-ranger").length) {
      ...
      $(".price-ranger .ranger-min-max-block .min").val(
         $(".price-ranger #slider-range").slider("values", 0) + " تومان"
      );
      $(".price-ranger .ranger-min-max-block .max").val(
       $(".price-ranger #slider-range").slider("values", 1) + " تومان" 
      );
    }
  }
 ...
 if ($(".marquee_mode").length) {
  ...
  if ($(".mobile-nav__container .main-menu__list").length) {
    ...
    dropdownAnchor.each(function () {
      ...
      toggleBtn.innerHTML = "<i class='fa fa-angle-up'></i>";
